...
description: Automatically initialize a selected blank project or subproject folder under the selected workspace root when the Enterprise Design Codex Plugin is selected. It creates missing project controls, inherits the latest plugin rules, protects business outputs, and routes product-design work through the enterprise workflow.
---

# Enterprise Design Project Bootstrap

Use this skill when the user selects the Enterprise Design Codex Plugin and provides a project folder under `<WORKSPACE_ROOT>`. The selected folder is the work target; the user does not need to repeat the skill instruction or run a command.

## Core behavior

1. Resolve the referenced file or folder and keep all writes inside `<WORKSPACE_ROOT>`.
2. Select the highest semantic-versioned directory under `<WORKSPACE_ROOT>\00-企业设计插件`.
...
8. For a product-design task, require the text confirmation, PRD generation, PRD comprehensive process/content validation, user answers or confirmation, PRD confirmation, and only then Demo generation.
9. For plugin, knowledge, template, tool, workspace, or project-control maintenance, route to the seven-scope maintenance flow instead of the product-Demo flow.
...
- Stop on a path outside `<WORKSPACE_ROOT>`, a non-empty nested folder without enough project evidence, a case-insensitive collision, a locked destination, or an existing destination that would require overwriting.
...
Read the nearest project `AGENTS.md`, the workspace `<WORKSPACE_ROOT>\AGENTS.md`, the active plugin, and the project context. Then follow the latest workflow, including the clarification gate, text confirmation, PRD comprehensive validation, user question confirmation, PRD confirmation, prototype comparison when applicable, Demo review, role/permission analysis, knowledge-base reference recording, and version control.
