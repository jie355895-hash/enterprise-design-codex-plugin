...
UI规范不以历史项目视觉值或中央知识库作为默认视觉来源。中央知识库只用于业务参考；UI视觉和交互标准主要参考Ant Design 5、Carbon Design System和WCAG 2.2，并由ui-ux-pro-max辅助视觉探索和评审。
...
17. 根据PRD生成Demo内说明
18. Demo自动检查与AI产品设计评审
19. 用户查看、点击和体验验收
20. 根据反馈迭代并保存PRD、原型和Demo版本
...
PRD生成后，必须先执行PRD综合校验，再向用户展示产品内容摘要、校验结果和疑问。PRD确认不是重新提问，而是确认文字稿的详细化结果及校验结论。只有用户回答疑问或确认无问题，并且没有未解决的高影响问题后，PRD才能标记为已确认并进入Demo生成。

## PRD综合校验与确认规则

PRD综合校验是一个环节，内部同时包含流程校验和内容校验，不得遗漏任一部分。

### 流程校验

- 页面入口、角色入口、租户入口和数据范围是否明确。
- 查看、编辑、审批、处理和导出等权限边界是否明确。
- 按钮前置条件、执行结果、状态变化和下一步去向是否明确。
- 正常流程、异常流程、空状态、加载、错误、无权限和不可操作状态是否完整。
- 状态是否存在无法到达、无法退出或责任人不明确的情况。
- 正常提醒与需处理提醒是否分类正确，需处理提醒是否具备处理闭环。
- 操作结果、审计记录和数据回写是否明确。
- 跨模块依赖、系统边界、数据主责和失败恢复是否明确。

### 内容校验

- 产品目标、范围、范围外内容、角色、租户、数据对象、字段和页面结构是否完整。
- 字段含义、数据来源、统计口径、单位、更新时间和只读/可编辑规则是否明确。
- 角色差异、数据权限、功能权限、流程权限和默认视图是否有依据。
- 说明 Drawer 是否覆盖页面、模块、区域、字段、按钮、状态、流程节点和指标中需要说明的对象。
- Demo视觉方向、交互要求、模拟范围、未模拟接口和验收标准是否足够生成Demo。
- 业务文档、截图、原型、菜单、权限、流程和文字确认稿之间是否存在冲突。

综合校验后必须输出：校验通过项、流程问题、内容问题、问题影响、依据来源、建议方案和待确认问题。问题数量没有固定限制，只提出影响产品判断的必要问题。

校验状态统一为：`草稿` → `PRD综合校验中` → `待确认` → `需修改` → `重新校验` → `已确认`。即使没有疑问，也必须输出校验结果并等待用户确认。

高影响问题包括角色、租户、数据范围、字段、权限、按钮、状态、核心流程、通知闭环和数据回写。高影响问题未解决时不得生成Demo。低影响问题可记录为默认假设，但必须写入PRD、Demo说明和版本记录。

PRD校验记录必须与PRD版本绑定，记录校验结果、问题、来源、用户回答、修改内容、确认状态和待处理事项。用户回答影响业务范围、角色、权限、字段、状态或流程时，必须创建新的PRD版本并重新校验，不能覆盖已确认PRD。
...
说明 Drawer 说明矩阵至少判断：当前租户/组织/工厂和数据范围、当前角色和权限、可见内容、字段含义和数据来源、按钮目的和前置条件、执行结果和状态变化、流程去向、禁用原因、风险和审计要求。说明内容必须只解释当前用户有权看到和操作的内容，不得成为绕过租户隔离或权限控制的入口。
...
- `v0.x`: PRD草稿、用户补充后待校验版本。
- `v1.0`: 首次完成综合校验并经用户确认的PRD版本。
- `v1.x`: 不改变产品目标、核心范围或主流程的确认后增量版本。
- `v2.0`: 产品目标、核心范围、租户、角色、权限、核心数据模型或主流程发生重大变化的版本。

PRD版本目录可以同时保存PRD文件、PRD综合校验记录和`version.json`。校验记录不替代用户确认，用户确认信息必须保留在版本记录中。
...
任务先分为产品设计任务和系统维护任务，两个流程不能混用。

产品设计任务执行需求、调研、澄清、业务分析、文字确认稿、PRD、PRD综合校验、Demo、评审和版本保存流程。

系统维护任务包括中央知识库、插件规则、项目创建模板、项目创建工具、同步工具、工作区上下文和子项目控制文件的更新。系统维护任务执行：识别目标 → 读取依据 → 输出变更方案 → 用户确认 → 修改目标范围 → 七范围影响评估 → 用户确认联动变更 → 备份 → 同步 → 验证 → 版本记录。

系统维护任务不自动生成PRD或Demo。若维护内容会改变产品设计流程、确认门槛、说明规则、版本规则或项目输出结构，必须评估并同步相关规则、模板、工作区上下文和项目控制文件。公开发布到GitHub属于维护后的独立发布动作，必须先完成脱敏检查和发布确认。
