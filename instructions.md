...
PRD生成后必须先完成一次综合校验，再向用户展示PRD摘要、校验结果和疑问。综合校验同时包含：

- 流程校验：入口、角色、租户、数据范围、权限、按钮条件、状态流转、异常流程、通知闭环、数据回写和跨模块依赖。
- 内容校验：目标、范围、角色、字段、数据来源、统计口径、说明 Drawer、Demo交互要求、验收标准、依据和待确认项。

校验结果必须分别列出通过项、流程问题、内容问题、问题影响、依据来源、建议方案和待确认问题。问题数量没有固定限制，只提出必要问题。

校验状态统一为：`草稿`、`PRD综合校验中`、`待确认`、`需修改`、`重新校验`、`已确认`。即使没有疑问，也必须等待用户确认。高影响问题包括角色、租户、数据、字段、权限、按钮、状态、核心流程、通知闭环和数据回写；高影响问题未解决时不得生成Demo。低影响问题可作为默认假设，但必须写入PRD、Demo说明和版本记录。

用户回答影响业务范围、角色、权限、字段、状态或流程时，创建新的PRD版本并重新校验，不得覆盖已确认PRD。PRD校验记录必须与PRD版本绑定，记录问题、依据、用户回答、修改内容、确认状态和待处理事项。

未完成综合校验或未确认PRD不得生成Demo。PRD是Demo页面、交互、状态、权限和页面内说明的产品依据。

PRD的说明Drawer必须按租户、公司/组织/工厂/实验室/部门数据范围、当前角色、可见内容、字段含义与来源、按钮用途与前置条件、操作结果、禁用原因、状态/流程上下文和风险/审计影响进行覆盖判断。只解释当前角色有权看到或操作的内容，不能通过说明Drawer绕过租户隔离、数据权限或操作权限。
...
说明覆盖矩阵还必须判断租户、公司/组织/工厂/实验室/部门和数据范围，当前角色与权限，可见内容，字段含义与来源，按钮用途、前置条件、结果和禁用原因，以及状态、流程、风险和审计影响。只对当前角色有权看到或操作的内容提供说明，不能通过说明Drawer绕过租户、数据或操作权限。

## 产品设计与系统维护分流

产品设计流程用于需求、PRD、Demo和验收；插件规则、中央知识库、项目模板、项目创建工具、同步工具、工作区上下文和子项目控制文件的修改使用独立的七范围维护流程，不生成PRD或Demo。

维护流程必须执行：

1. 识别目标范围并读取依据。
2. 输出目标变更方案、依据、版本影响和明确不变更项。
3. 等待用户确认后修改目标范围。
4. 修改后评估其他六个范围，分类为必须更新、建议更新、无需更新或待确认。
5. 对可能联动的范围输出具体变更内容并等待确认。
6. 备份项目、按依赖顺序更新、保留历史版本，只同步已确认的项目控制文件。
7. 验证版本指向、规则一致性、项目初始化和受保护业务输出未被改写。

如果维护内容改变需求澄清、文字确认、PRD校验、Demo门槛、说明Drawer、版本管理或交付目录规则，必须同步评估所有受影响范围。公开发布到GitHub还必须单独完成脱敏检查和发布确认，不能默认随本地维护自动公开。
